...
use App\Models\{Invoice, Purchase, InvoiceItem};
...
use App\Http\Controllers\{AdminController, ExpenseController, GoogleContactController, InvoiceController, ProfileController, PurchaseController, ReportController, RoleController, SaleController, TransactionController, UserController, PartyController};
...
Route::middleware(['auth', 'permission:dashboard.view'])->get('/dashboard', function (Request $request) {
    ...
    $numberOfProductsSoldInMonth = InvoiceItem::whereBetween('created_at', [$startOfMonth, $endOfMonth])->where('deleted', 0)->count();
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/admin/purchase/getparties', [PurchaseController::class, 'getParties'])->name('purchase.getchparties');
});
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/admin/invoice/download-pdf/{id}', [InvoiceController::class, 'downloadPdf'])->middleware('permission:invoices.print')->name('download-pdf');
    ...
    Route::get('/admin/invoice/send-whatsapp/{id}', [InvoiceController::class, 'sendWhatsapp'])->name('send-whatsapp');
});

// Public Routes
Route::get('/public/invoice/pdf/{id}', [InvoiceController::class, 'publicPdf'])->name('public-invoice-pdf')->middleware('signed');
...
// Party
Route::middleware(['auth'])->prefix('admin/parties')->name('parties.')->group(function () {
    Route::get('/', [PartyController::class, 'index'])->middleware('permission:parties.view')->name('listparties');
    Route::get('/add', [PartyController::class, 'addParty'])->middleware('permission:parties.create')->name('create');
    Route::get('/partydetail/{id}', [PartyController::class, 'detailParty'])->middleware('permission:parties.viewdetail')->name('create');
    Route::get('/edit/{id}', [PartyController::class, 'editParty'])->middleware('permission:parties.edit')->name('edit');
    Route::post('/store', [PartyController::class, 'saveParty'])->middleware('permission:parties.store')->name('store');
    Route::post('/delete/{id}', [PartyController::class, 'deleteParty'])->middleware('permission:parties.delete')->name('delete');
});
