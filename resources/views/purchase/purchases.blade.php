            ...
            <div class="col-12 col-sm-12 col-md-3">
                <div class="card card-pricing" style="background: #08b14a; color:#fff">
                    <div class="card-body text-center" style="padding: 0.5rem;">
                        ...
                        <h3 class="text-white">{{config('constant.currency')}} {{ number_format($totalAvailableStockAmount, 2) }}</h3>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    @if (count($allPurchases))
                        ...
                        <form action="{{ route('allpurchases') }}" method="GET" id="filterpurchase">
                            <div class="form-row">
                                ...
                                <button class="form-group col-md-1 btn btn-sm btn-danger" type="button" onclick="clearFilters()">Clear</button>
                            </div>
                            {{-- clear button --}}
                        </form>
                                ...
                                <tbody>
                                    @foreach($allPurchases as $key => $purchase)
                                        <tr>
                                            ...
                                            <td onclick="copyToClipboard('{{ $purchase->imei }}')" style="cursor: pointer;" title="Click to Copy">
                                                {{ $purchase->imei }} </td>
                                            ...
                                            <td class="text-right">
                                                ...
                                                <form method="post" action="{{ route('delete-stock', $purchase->id) }}"
                                                    ...
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-xs btn-danger" onclick="sureToDelete(event)" title="Delete">
