...
class PurchaseController extends Controller
{
    public function index(Request $request)
    {
        ...
        $colors = Purchase::select('color')
            ...
            ->get();
        $totalAvailableStockAmount = Purchase::query()
                ->where('deleted', 0)
                ->where('is_sold', 0)
                ->sum('purchase_cost');
        return view('purchase.purchases', [
            ...
            'colors'        => $colors,
            'selectedcolor' => $color,
            'totalAvailableStockAmount' => $totalAvailableStockAmount,
        ]);
    }
    ...
    public function savePurchase(Request $request)
    {
        ...
        $purchase->storelocation    = $request->storelocation;
        $purchase->model            = $request->model;
        $purchase->imei             = $request->imei;
        $purchase->msp              = $request->msp;
    ...
    private function saveMultiplePurchases(Request $request)
    {
        $request->validate([
            ...
            'stock_items.*.warrentydate'  => 'nullable|date',
            'stock_items.*.purchase_date' => 'required|date'
        ]);
        ...
        $message = count($purchasesData) . " stock items added successfully | (Total Cost: ₹" . number_format($totalCost, 2) . ")";
