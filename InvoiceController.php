...
use App\Models\{Invoice, InvoiceItem, Purchase};
use Carbon\Carbon;
use Exception;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\{DB, Auth, Storage, URL};
...
class InvoiceController extends Controller
{
    ...
    public function newInvoice()
    {
        ...
        if ($lastInvoice) {
            ...
            preg_match('/INVOICE(\d{4})/', (string) $lastInvoice->invoice_no, $matches);
        ...
        $formattedNumber = 'INVOICE' . $currentYear . str_pad($nextInvoiceNo, 4, '0', STR_PAD_LEFT);
    ...
    public function createInvoice(Request $request)
    {
        ...
        if ($request->warranty_expiry_date) {
            try {
                ...
                return back()->withErrors(['error' => 'Invalid warranty expiry date format. Please use dd/mm/yyyy format.'])->withInput();
            }
        }
    ...
    public function downloadPdf($id)
    {
        ...
        $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('invoice.pdf', ['invoice' => $invoice, 'amountInWords' => $amountInWords]);
    ...
    public function sendWhatsapp($id)
    {
        $invoice       = Invoice::with('items.purchase')->findOrFail($id);
        $amountInWords = $this->amoutInWords(floatval($invoice->net_amount));
        // $fileName = $invoice->invoice_no.'-'.Str::random(10).'.pdf';
        ...
        Your invoice total is ₹" . number_format($invoice->net_amount, 2) . ".\n\nYou can view and download your digital invoice here:\n" . $url;
    ...
    public function fetchModelData($imei)
    {
        ...
        //     'purchase' => $purchase,
        // ]);
        $query = Purchase::where('imei', 'LIKE', $imei . '%')
                    ->where('is_sold', 0)
                    ->where('deleted', 0)
                    ->take(10)
                    ->get();
